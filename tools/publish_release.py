#!/usr/bin/env python3
"""Publish a reviewed set of atlas claims as one reproducible non-canonical release.

Dry-run is the default. --apply changes publication_status and writes the
release manifest/source-version membership in one transaction.
"""
...
def load_manifest(path: Path) -> dict[str, Any]:
    ...
    for key in ("release_version", "schema_version", "claim_ids", "changelog", "qc_summary", "unresolved_issues"):
        if key not in data:
            raise PublishError(f"{key} is required")
    if not isinstance(data["claim_ids"], list) or not data["claim_ids"]:
        raise PublishError("claim_ids must be a non-empty array")
    if len(data["claim_ids"]) != len(set(data["claim_ids"])):
        raise PublishError("claim_ids must be unique")
    if data.get("canonical", False):
        raise PublishError("this tool is for non-canonical preview/public releases only")
    return data


def inspect_release(cur, claim_ids: list[str]) -> dict[str, Any]:
    cur.execute(
        """
...
def publish(conn, manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    with conn.cursor() as cur:
        check = inspect_release(cur, claim_ids)
        if check["problems"]:
        ...
        geometry_ids = cur.fetchone()[0] or []
        ...
        source_version_ids = cur.fetchone()[0] or []

        release_payload = {
            "canonical": False,
            "purpose": manifest.get("purpose", "public_mvp_preview"),
            "claim_ids": sorted(claim_ids),
            "spatial_entity_ids": sorted(spatial_ids),
...
def main() -> int:
    ...
    if not args.dsn:
        ...
        print(json.dumps({
            ...
            "canonical": False,
            "mode": "syntax-only dry run; supply DATABASE_URL for database gate checks",
        }, indent=2))
        return 0
    ...
    with psycopg.connect(args.dsn, autocommit=False) as conn:
        with conn.cursor() as cur:
            check = inspect_release(cur, manifest["claim_ids"])
        if check["problems"]:
            for problem in check["problems"]:
                print(f"BLOCK: {problem}", file=sys.stderr)
            return 2
        print(f"READY: {len(manifest['claim_ids'])} claims pass the publication gate")
        if not args.apply:
            conn.rollback()
