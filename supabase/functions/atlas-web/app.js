(() => {
  const API_URL = "/functions/v1/atlas-data";
  ...
  const activeInYear = (fromYear, toYear, year) =>
    (fromYear === null || year >= fromYear) && (toYear === null || year <= toYear);

  const formatYear = (year) => year <= 0 ? `${1 - year} BCE` : `${year} CE`;

  const formatInterval = (fromYear, toYear) => {
  ...
  const activeClaims = (place, year) => place.claims.filter((claim) => activeInYear(claim.from_year, claim.to_year, year));
  const geometryForYear = (place, year) =>
    place.geometries.find((geometry) => geometry.geometry !== null && activeInYear(geometry.from_year, geometry.to_year, year)) ?? null;
  const unresolvedGeometryForYear = (place, year) =>
    place.geometries.find((geometry) => geometry.accuracy_status === "unresolved" && activeInYear(geometry.from_year, geometry.to_year, year)) ?? null;

  const highestPracticeLevel = (claims) => claims.reduce((max, claim) => {
    if (!claim.practice_level) return max;
    return Math.max(max, Number(claim.practice_level.slice(1)));
  }, 0);

  const featureState = (claims) => {
    if (claims.length === 0) return "inactive";
    return claims.some((claim) => claim.coverage_state !== "disputed") ? "supported" : "disputed";
  };

  const buildFeatureCollection = (year) => ({
    ...
    const sources = claim.sources.length
      ? `<ul class="source-list">${claim.sources.map((source) => {
          ...
          const author = source.author_or_institution
            ...
            : locator ? `<div class="meta source-meta">${locator.slice(3)}</div>` : "";
  ...
  const renderPlace = (place, year) => {
    ...
    panel.innerHTML = `
      ...
      <div class="geometry-note">
        ...
        ${escapeHtml(geometry?.resolution_method ?? unresolved?.resolution_method ??
    ...
    `;
    document.querySelector("#back-overview")?.addEventListener("click", () => renderOverview(year));
  };

  const renderOverview = (year) => {
    ...
    panel.querySelectorAll("[data-place-id]").forEach((button) => {
  ...
  const updateMap = (year) => {
    ...
    map.getSource("evidence")?.setData(buildFeatureCollection(year));
    ...
    const selected = selectedPlaceId ? places.find((place) => place.spatial_entity_id === selectedPlaceId) : null;
    selected ? renderPlace(selected, year) : renderOverview(year);
  };

  const currentYear = () => Number(slider.value);

  const pickFeature = (event) => {
    const id = event.features?.[0]?.properties?.spatial_entity_id;
  ...
  const boot = async () => {
    try {
      ...
      const years = places.flatMap((place) =>
        place.claims.flatMap((claim) => [claim.from_year, claim.to_year].filter((year) => year !== null))
      );
      ...
      map.addSource("land", { type: "geojson", data: "/functions/v1/atlas-web/world-land.geojson" });
      map.addLayer({ id: "land-fill", type: "fill", source: "land", paint: { "fill-color": "#efeee9", "fill-opacity": 1 } });
      map.addLayer({ id: "land-line", type: "line", source: "land", paint: { "line-color": "#8f8b83", "line-width": 0.65 } });

      map.addSource("evidence", { type: "geojson", data: buildFeatureCollection(currentYear()) });
      map.addLayer({
        id: "evidence-fill", type: "fill", source: "evidence",
        filter: ["==", ["geometry-type"], "Polygon"],
        paint: {
          "fill-color": ["match", ["get", "state"], "supported", "#9e493f", "disputed", "#c18a31", "#b8b7b2"],
          "fill-opacity": ["case",
      ...
      map.addLayer({
        id: "evidence-line", type: "line", source: "evidence",
        filter: ["==", ["geometry-type"], "Polygon"],
        paint: {
          "line-color": ["match", ["get", "state"], "supported", "#6e2b24", "disputed", "#815914", "#8f8d88"],
          "line-width": ["case", ["==", ["get", "state"], "inactive"], 0.7, 1.5],
        },
      });
      map.addLayer({
        id: "evidence-points", type: "circle", source: "evidence",
        filter: ["==", ["geometry-type"], "Point"],
        paint: {
          "circle-radius": ["case", ["==", ["get", "state"], "inactive"], 4, 7],
          "circle-color": ["match", ["get", "state"], "supported", "#9e493f", "disputed", "#c18a31", "#b8b7b2"],
          "circle-opacity": ["case",
      ...
      ["evidence-fill", "evidence-points"].forEach((layer) => {
