-- Migration 0019: adaptive coastal completion for coarse historical polygons.
-- Extends the precomputed render cache with a bounded, topology-aware recovery tier.
-- No expensive geometry work occurs in the public request path.
...
ALTER TABLE cartography.geometry_render_policy
    ADD COLUMN max_coastal_recovery_m integer NOT NULL DEFAULT 0
        CHECK (max_coastal_recovery_m BETWEEN 0 AND 100000),
    ADD COLUMN max_extra_recovery_pct numeric(8,4) NOT NULL DEFAULT 0
        CHECK (max_extra_recovery_pct BETWEEN 0 AND 25);

ALTER TABLE cartography.render_geometry_cache
    ADD COLUMN coastal_recovery_m_used integer,
    ADD COLUMN coastal_extra_area_pct double precision;
...
INSERT INTO cartography.geometry_render_policy(
    ...
    'cliopatria-boundary-normalization-v4',
    ...
    3,
    true,
    'Adaptive coastal completion for coarse Cliopatria polygons. Uses two bounded smoothing iterations, then selects the largest recovery tier up to 50 km whose added land versus the 25 km baseline remains <=2% of the baseline display area. Source geometry remains immutable.'
);
...
SELECT
    ...
    CASE WHEN c.geom IS NOT NULL THEN c.coastal_recovery_m_used ELSE NULL END AS render_coastal_recovery_m,
    ...
    CASE WHEN c.geom IS NOT NULL THEN
        jsonb_build_object(
            ...
            'coastal_extra_area_pct', c.coastal_extra_area_pct,
