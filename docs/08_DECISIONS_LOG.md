...
**Consequences:** migrations `0028_claim_kind_integrity.sql` and `0029_claim_kind_function_privileges.sql`, plus the regression test, are part of the append-only database foundation. New specialized claim types must define and test their claim-kind boundary and preserve D-051's private-function privilege boundary. Existing release/data semantics remain unchanged; M2 post-M1 semantic integration still proceeds under D-058 rather than being inferred from this guard.
