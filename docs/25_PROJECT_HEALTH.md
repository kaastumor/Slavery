...
| A-013 | A real bounded release programme can harden the surviving method better than further miniature architecture experiments | survives; cumulative internal review completed | The 17-row post-R1 tranche replay produced 12 accepted bounded evidence states and 5 explicit HOLDs without new research. Source reconstructibility passed across 113 relations, with one cross-row dependency family preserved. No independent review exists, so automatic publication/intake is not justified. | Preserve the internal candidate; move to maintenance/trigger-bound review until a D-097 reopening condition occurs. |
...
| R-004 | Governance/document drift | medium / active but narrowed by D-096 | Repeated pointer-repair PRs showed that duplicated mutable scheduling state was not solved. BACKLOG now owns priority/mode; active checkpoint owns within-item progress; README/status are summaries only. |
