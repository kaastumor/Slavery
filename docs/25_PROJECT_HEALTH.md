...
| A-013 | A real bounded release programme can harden the surviving method better than further miniature architecture experiments | survives; R1 completed below promotion ceiling | R1 demonstrated ordinary research → replay → QC → hold/release decision. Later EXP work added useful evidence but remained experiment-local. D-096 therefore makes cross-batch consolidation/review capacity the next pressure before more intake. | Reconcile post-R1/EXP research artifacts before sustained scaling or a successor release; preserve remaining queue as research-state, not absence. |
